...
%
% {
%   "target_subsection": "Multiple (Sec 4, Sec 6, App A)",
%   "reasons": [
%     "Resolve a critical structural error by consolidating the duplicated Appendix A, improving notation hygiene and clarity.",
%     "Improve mathematical rigor and verification coverage by completing the Lean4 proof for Hamiltonian convexity (Sec 4), removing a TODO marker.",
%     "Improve didactic clarity and visual flow by relocating Figure 3 (Isoelastic Demand) from Section 2 to Section 6 where it is contextually relevant."
%   ],
%   "build_notes": {
...
-- sum_i (P(Y1)-P(Y2)) * q_i * (w1_i - w2_i) = (P(Y1)-P(Y2)) * (Y1 - Y2) <= 0 for antitone P.
variable {iota : Type*} [Fintype iota]

theorem ll_mono_discrete (q : iota → ℝ) (P : ℝ → ℝ)
  (w1 w2 : iota → ℝ) (hP : Antitone P) :
  sum i, (P (sum j, q j * w1 j) - P (sum j, q j * w2 j)) * q i * (w1 i - w2 i)
    <= 0 := by
  classical
  set Y1 : ℝ := sum j, q j * w1 j
  set Y2 : ℝ := sum j, q j * w2 j
  have hfactor : sum i, (P Y1 - P Y2) * (q i * (w1 i - w2 i))
      = (P Y1 - P Y2) * (sum i, (q i * (w1 i - w2 i))) := by
    simpa [mul_sum, sum_mul, mul_comm, mul_left_comm, mul_assoc]
  have hsum : (sum i, q i * (w1 i - w2 i)) = Y1 - Y2 := by
    simp [Y1, Y2, mul_sub, Finset.sum_sub_distrib]
  have : sum i, (P Y1 - P Y2) * q i * (w1 i - w2 i)
      = (P Y1 - P Y2) * (Y1 - Y2) := by
    simpa [hfactor, hsum, mul_comm, mul_left_comm, mul_assoc]
  -- Apply antitone cross-product sign lemma from above.
...
lemma cross_minus_sorted_eq (a1 a2 b1 b2 : ℝ) :
...
-- Prove: p ↦ a*p^2 + b*p + c is convex on ℝ if a ≥ 0.
