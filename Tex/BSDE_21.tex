...
We adapt the deep forward--backward methodology developed in Ji Huang's FBSDE studies\footnote{\url{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4538048},\,\url{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4122454},\,\url{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5199943},\,\url{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4742398},\,\url{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4649043}} to the canonical two-tree Lucas economy with a single log-utility investor. Let $D^i_t$ denote the dividend of tree $i$ and write $X^i_t=\log D^i_t$. The forward state collects $\mathbf{X}_t=(X^1_t,X^2_t)^	op$ and evolves as coupled Ornstein--Uhlenbeck diffusions,
egin{align}
\diff X^i_t &= \kappa_iig(ar X_i - X^i_tig)\,\diff t + \sigma_i\,\diff W^i_t,\qquad i=1,2,\
\diff\langle W^1,W^2angle_t &= ho\,\diff t,\qquad D^i_t = e^{X^i_t},\qquad C_t = D^1_t + D^2_t.
\end{align}
Consumption growth inherits drift $\mu_C(\mathbf{X}_t)=\sum_{i=1}^2 D^i_t[\kappa_i(ar X_i - X^i_t)+	frac12\sigma_i^2]$ and diffusion vector $\sigma_C(\mathbf{X}_t)=(D^1_t\sigma_1,\,D^2_t\sigma_2)^	op$, delivered by the correlated Brownian motions $W^1$ and $W^2$ with instantaneous correlation $ho$.

\paragraph{Backward block (log-utility pricing).}
Log preferences imply the stochastic discount factor $M_t=e^{-eta t}/C_t$ and market price of risk $	heta_t=\sigma_C(\mathbf{X}_t)/C_t$. Denoting the cum-dividend value of tree $i$ by $P^i_t$, we normalise by consumption and set $Y^i_t=P^i_t/C_t$. Collect the backward variables $\mathbf{Y}_t=(Y^1_t,Y^2_t)^	op$ and note that $\mathbf{Z}_t\in\mathbb{R}^{2	imes 2}$; they solve the vector FBSDE
egin{align}
\mathbf{Y}_T &= \mathbf{0},\
\diff \mathbf{Y}_t &= -\Big[eta\,\mathbf{Y}_t - rac{\mathbf{D}_t}{C_t} + \mathbf{Z}_t\,	heta_t\Big]\,\diff t + \mathbf{Z}_t\,\diff \mathbf{W}_t,\qquad \mathbf{D}_t=(D^1_t,D^2_t)^	op,\,\mathbf{W}_t=(W^1_t,W^2_t)^	op,
\end{align}
over a large but finite horizon $[0,T]$. The driver enforces the Euler pricing condition: the $eta\,\mathbf{Y}_t$ term discounts future payoffs, $\mathbf{D}_t/C_t$ injects the dividend flow, and $\mathbf{Z}_t\,	heta_t$ captures hedging cash flows generated by trading the two trees. The transversality condition of the infinite-horizon problem is implemented numerically by taking $T$ so that $e^{-eta T}$ is negligible and prescribing the zero terminal condition above.

\paragraph{Numerics.}
Following the cited FBSDE program, we discretise $[0,T]$ on a uniform grid $0=t_0<\dots<t_N=T$ and sample forward paths of $\mathbf{X}_{t_n}$ with antithetic Gaussian or Sobol increments. A residual network $\mathcal{N}_	heta(t_n,\mathbf{X}_{t_n})$ parameterises the hedge matrix $\mathbf{Z}_{t_n}$, while the normalised value $\mathbf{Y}_{t_n}$ is updated with an implicit Euler step consistent with the driver above. Mini-batch stochastic gradient descent minimises the terminal mismatch $\|\mathbf{Y}_T\|^2$ plus pathwise Euler residuals. Once $\mathbf{Z}_	heta$ converges, tree prices recover through $P^i_t=C_t Y^i_t$; martingale diagnostics on $M_t P^i_t + \int_0^t M_s D^i_s\,\diff s$ and stability checks in $T$, $ho$, and network depth mirror the replication checklist, ensuring a robust two-tree solution within the multicountry solver framework.
%========================
