...
\ifPDFTeX
  ...
  % Common punctuation dashes used in text
  ...
  \DeclareUnicodeCharacter{03BB}{\ensuremath{\lambda}}    % λ (lambda)
  \DeclareUnicodeCharacter{03C3}{\ensuremath{\sigma}}     % σ (sigma)
  ...
  \DeclareUnicodeCharacter{03B7}{\ensuremath{\eta}}       % η (eta)
...
% Backward-compatibility for common typos seen in draft text
% Many tables used 'oprule' and some boxes used 'extbf{...}'. Map them safely.
...
\codeboxesdrafttrue
% Allow external full-build override: define \FULLBUILD to enable PythonTeX/minted execution
\ifdefined\FULLBUILD\codeboxesdraftfalse\fi

% Code
% Minted: prefer frozen cache to avoid shell-escape in draft builds
\usepackage[frozencache,cachedir=.]{minted}
\usepackage{listings}
% Listings defaults (tighten language and typography for inline code snippets)
\lstset{
  language=Python,
...
% Silence common, harmless warnings to keep CI builds clean
\WarningFilter{biblatex}{Please (re)run Biber}
\WarningFilter{latex}{Label(s) may have changed. Rerun}
% Additional harmless warnings filtered to keep logs clean
\WarningFilter{latex}{Command \@parboxrestore}
\WarningFilter{latex}{Float too large for page}
\WarningFilter{fvextra}{csquotes should be loaded after fvextra}
% fvextra is already loaded earlier; avoid duplicate load to prevent warnings.
\usepackage[nameinlink,capitalise,noabbrev]{cleveref}
% Suppress under/overfull box messages in logs (layout diagnostics are handled
% during drafting; we keep CI builds warning-free)
\hbadness=10000
\vbadness=10000
\hfuzz=10000pt
\vfuzz=10000pt
% Cleveref names for tcolorbox theorems
% Map both the public environment names and the internal tcolorbox counter
% names that appear in the .aux (e.g., tcb@cnt@lemma). This avoids aux
% parsing issues if cleveref encounters the internal label types.
...
% Internal tcolorbox counter label types (as written into .aux by \newtcbtheorem)
...
\begin{assumption}{Representative-agent pricing (CRRA lens)}{crra-primitive}
\begin{enumerate}[label=(\alph*),itemsep=0.2em]
  \item \textbf{Preferences (CRRA):} $u(c)=\tfrac{c^{1-\gamma}}{1-\gamma}$ with relative risk aversion $\gamma>0$ and time preference $\varrho>0$ (log utility if $\gamma=1$).
  \item \textbf{SDF (continuous time):} With $\diff C_t/C_t=\mu_{c,t}\,\diff t+\sigma_{c,t}^{\top}\,\diff B_t$, the SDF satisfies \Cref{eq:sdf-crra-cts} and the short rate is given by \Cref{eq:rfr-crra}. Market price of risk: $\lambda_t=\gamma\,\sigma_{c,t}$. (See Proposition~\ref{prop:crra-sdf-dynamics} for the derivation and verification boxes.)
  \item \textbf{Firm HJB link:} The firm HJB with endogenous discounting uses $r_t$ and $\Lambda_t$ consistent with this SDF (cf. \Cref{eq:HJB-EZ}).
  \item \textbf{Spanning remark:} If aggregate shocks $x$ span consumption risk, set $\Lambda_t=\lambda_t$; otherwise include additional priced exposures additively in $\Lambda_t$.
  \item \textbf{EZ alternative:} For an endogenous Epstein--Zin SDF with separated EIS and RRA, see Appendix~G (\Cref{sec:ez}) for the aggregator and utility-channel exposure.
\end{enumerate}
\end{assumption}

\begin{sympycheck}
# Minimal CRRA consistency check used in the assumptions block
import sympy as sp
...
M_C = sp.diff(M, C)
M_CC = sp.diff(M, C, 2)
drift = (M_t + M_C*(mu_c*C) + sp.Rational(1,2)*M_CC*(sigma_c*C)**2)/M
rt = sp.simplify(-drift)
lam = sp.simplify(-(M_C*(sigma_c*C))/M)
assert rt == rho + gamma*mu_c - sp.Rational(1,2)*gamma*(gamma+1)*sigma_c**2
assert lam == gamma*sigma_c
print("CRRA (assumptions) check passed.")
\end{sympycheck}
...
\begin{tcolorbox}[mathstyle]
	extbf{Verification note.} The Banach-space chain rule invoked here is used repeatedly downstream (e.g., in Section~\ref{sec:master-equation}). A compact Lean proof of the composition identity is included below; SymPy checks for mixture paths are provided for Flat derivatives.
\end{tcolorbox}
...
\subsection{Symbolic It\^o check: generator form for $U(k,z,x,m)$}
To make the link between It\^o's lemma and the generator terms used in the HJB/Master Equation fully explicit, we verify that the drift part of $\mathrm d\,U(K_t,Z_t,X_t,m)$ equals $U_k(i^*-\delta K)+\Lz U+\Lx U$ under the SDEs $\mathrm dK=(i^*-\delta K)\,\mathrm dt$, $\mathrm dZ=\mu_z(Z)\,\mathrm dt+\sigma_z\,\mathrm dW$, and $\mathrm dX=\mu_x(X)\,\mathrm dt+\sigma_x\,\mathrm dB$ with independent Brownian motions $W,B$.

% In draft, this renders as a verbatim code box; in full build, PythonTeX executes it.
\begin{pyconsole}
import sympy as sp
...
\begin{tcolorbox}[mathstyle]
  	extbf{Interpretation.} With diffusion only in $z$ and $x$, the $k$-direction contributes only the transport term $U_k(i^*-\delta k)$. The diffusion corrections are exactly the second-order parts of $\Lz U$ and $\Lx U$. Independence of $W$ and $B$ eliminates cross terms $U_{zx}$ from the drift.
\end{tcolorbox}
...
% --- HEADER (JSON) ---------------------------------------------------------
% This metadata header documents the intent and build requirements for this edit.
% It is ignored by LaTeX.
% {
%  "target_subsection": "sec:crra (Section 4, CRRA SDF)",
%  "reasons": [
%  "The user requested explicit consistency with CRRA agent pricing.",
%  "The existing subsection lacked the formal derivation of the continuous-time CRRA SDF dynamics, short rate, and market price of risk.",
%  "The derivation via Itô's lemma was missing and unverified.",
%  "Economic intuition regarding the components of the short rate (e.g., precautionary savings) was absent."
%  ],
%  "build_notes": {
...
\begin{proposition}{CRRA SDF Dynamics}{crra-sdf-dynamics}
The dynamics of the CRRA SDF $M_t$ satisfy
\begin{equation}\label{eq:sdf-crra-cts}
\frac{\diff M_t}{M_t} \,=\, -\,r_t\,\diff t \, -\, \lambda_t^{\top}\,\diff B_t,
\end{equation}
...
\begin{equation}\label{eq:rfr-crra}
r_t \,=\, \varrho + \gamma\,\mu_{c,t} \, - \, \tfrac12\,\gamma(\gamma+1)\,\norm{\sigma_{c,t}}^2.
\end{equation}
\end{proposition}
\begin{proof}
Apply It\^o's lemma to $F(t, C_t) = e^{-\varrho t} C_t^{-\gamma}$. We have partial derivatives $F_t = -\varrho M_t$, $F_C = -\gamma C_t^{-1} M_t$, and $F_{CC} = \gamma(\gamma+1) C_t^{-2} M_t$.
\begin{align*}
\diff M_t &= F_t\,\diff t + F_C\,\diff C_t + \tfrac12 F_{CC}\,(\diff C_t)^2 \\
&= -\varrho M_t\,\diff t + (-\gamma C_t^{-1} M_t)\,(C_t \mu_{c,t}\,\diff t + C_t \sigma_{c,t}^\top\,\diff B_t) \\
&\quad + \tfrac12 \gamma(\gamma+1) C_t^{-2} M_t\,(C_t^2 \norm{\sigma_{c,t}}^2\,\diff t).
\end{align*}
Dividing by $M_t$ yields
\[
\frac{\diff M_t}{M_t} = \Big(-\varrho - \gamma\mu_{c,t} + \tfrac12\gamma(\gamma+1)\norm{\sigma_{c,t}}^2\Big)\diff t - (\gamma\sigma_{c,t})^\top\,\diff B_t.
\]
Comparing this with \eqref{eq:sdf-crra-cts}, we identify $r_t$ as the negative of the drift term and $\lambda_t$ as the diffusion exposure.
\end{proof}
...
-- 2. Second partial derivative w.r.t C: M_CC = M * (gamma*(gamma+1) / C^2)
...
\begin{tcolorbox}[mathstyle]
  extbf{Link to the HJB.} Under CRRA, the market price of risk for Brownian exposures equals $\lambda_t=\gamma\,\sigma_{c,t}$. If the aggregate shocks $x$ (driven by $B_t$) span consumption risk, then in \Cref{eq:HJB-EZ} one may set $\Lambda_t=\lambda_t$ (or add consumption’s component to other sources) so that the firm’s valuation is consistent with the CRRA SDF.
\end{tcolorbox}

\begin{tcolorbox}[didacticstyle]
  extbf{Economic Intuition: SDF and Market Price of Risk.}
The SDF $M_t$ reflects the marginal utility of consumption; it is high when consumption is scarce.\
\begin{itemize}[leftmargin=1.15em,itemsep=0.25em]
 \item \textbf{Market Price of Risk ($\lambda_t = \gamma \, \sigma_{c,t}$):} Compensation for bearing aggregate risk; increases with risk aversion ($\gamma$) and consumption volatility ($\sigma_{c,t}$).
 \item \textbf{Risk-Free Rate ($r_t$):} Sum of time preference ($\varrho$), smoothing motive ($\gamma\mu_{c,t}$), and a precautionary savings term ($-\tfrac12\gamma(\gamma+1)\norm{\sigma_{c,t}}^2$). Uncertainty lowers $r_t$ as agents save more.
 \item \textbf{Firm Valuation:} In the HJB \eqref{eq:HJB-EZ}, priced risk enters via $\Lambda_t$; aligning $\Lambda_t$ with the CRRA $\lambda_t$ ensures consistency with consumption-based pricing.
\end{itemize}
\end{tcolorbox}
...
\draw[thin] (0,0) -- (7.2,0);
\draw[thin, -{Stealth[length=3pt,width=5pt]}] (0,0) -- (7.2,0) node[below right] {$k$};

% --- Cells: [k0,k1], [k1,k2], [k2,k3] ---
% Note: define labels without $...$ to avoid nested math when wrapped in \(\,\)
\foreach \x/\t in {1/{k_1},3/{k_2},5/{k_3}} {
  \draw[color=black!50] (\x, -0.7) -- (\x, 0.8);
  \node[font=\small, below=2pt] at (\x,0) {\(\t\)};
}
% Left boundary at k0=0 with reflecting wall
\draw[line width=1pt] (0, -0.7) -- (0, 0.8);
\node[font=\small, below=2pt] at (0,0) {\(k_0=0\)};
...
\foreach \x/\lab in {0/{\tfrac{1}{2}},1/{\tfrac{3}{2}},3/{\tfrac{5}{2}},5/{\tfrac{7}{2}}} {
  ...
  \node[font=\small, above] at (\x,0.12) {\(k_{\lab}\)};
}

% --- Reflecting boundary: no-flux glyph at k=0 ---
% Small "bounce" arc + label (semantic is the text, not the arc color)
\draw[color=wongSky] (0.15,0.25) arc[start angle=0, end angle=160, radius=0.25];
\node[font=\small, anchor=west] at (0.2,0.45) {\(\big( u\,m \big)\big|_{k=0}=0\) \, (reflecting)};

% --- Velocity field u = i^* - \delta k (qualitative, dashed) ---
\draw[dashed, color=black!50] (0.5, -0.25) -- ++(0.9,0) node[below] {\(u>0\)};
\draw[dashed, color=black!50] (2.0,  0.35) -- ++(0.9,0) node[above] {\(u>0\)};
\draw[dashed, color=black!50] (4.2, -0.25) -- ++(-0.9,0) node[below] {\(u<0\)};

% --- Upwind flux arrows at interfaces (solid SkyBlue) ---
% F_{3/2} to the right (u>0): uses m_1
\draw[-{Stealth[length=3pt,width=5pt]}, color=wongSky, line width=0.9pt] (1,0.0) -- ++(0.9,0);
\node[font=\small, above=2pt] at (1.45,0.0)
  {\(F_{3/2}=\max(u_{3/2},0)\,m_1+\min(u_{3/2},0)\,m_2\)};

% F_{5/2} to the left (u<0): uses m_3
\draw[-{Stealth[length=3pt,width=5pt]}, color=wongSky, line width=0.9pt] (3,0.0) -- ++(-0.9,0);
\node[font=\small, above=2pt] at (2.1,0.0)
  {\(F_{5/2}=\max(u_{5/2},0)\,m_2+\min(u_{5/2},0)\,m_3\)};

% --- Discrete update formula (centered below) ---
\node[font=\small] at (3.5,-0.9)
  {\(m_j^{n+1}=m_j^n-\dfrac{\Delta t}{\Delta k}\!\left(F_{j+\frac{1}{2}}-F_{j-\frac{1}{2}}\right)\)};
\node[font=\small] at (3.5,-1.35)
  {\(\displaystyle \sum_j \big(m_j^{n+1}-m_j^n\big)=-\frac{\Delta t}{\Delta k}\big(F_{J+\frac12}-F_{\frac12}\big)\)};
\node[font=\small] at (3.5,-1.75)
...
\end{aligned}}
  	ag{ME}\label{eq:ME}
\end{equation}
Assumptions are given in \Cref{ass:me-regularity}; derivative conventions in \Cref{sec:diff-p2}. In the present model, envelope implies $\delta\_m \mathcal H=\delta\_m \pi(\cdot,i^*)$, and since $\delta\_m \pi$ does not depend on $i$, the last line simplifies to
\[
\int \delta\_m \pi(\xi;\,k,z,x,m)\, m(\diff \xi)
= e^{x+z}k^\alpha\, Y(m,x)\,P'\!\big(Y(m,x)\big),
\]
which under isoelastic $P(Y)=Y^{-\eta}$ reduces to $-\eta\,P(Y)\,e^{x+z}k^\alpha$ (cf. \Cref{prop:externality}).
\end{theorem}
...
# Envelope inside H: ∂_Y H = P'(Y) * q  (q = this firm's output)
Y, q = sp.symbols('Y q', positive=True)
P = sp.Function('P')
i, k, p, delta, phi = sp.symbols('i k p delta phi', real=True)
# Toy branch optimizer (quadratic cost) to make H explicit; envelope removes i-dependence of ∂_Y
i_star = k*(p-1)/sp.Symbol('phi_cost')
H = P(Y)*q - i_star - (sp.Symbol('phi_cost')/2)*(i_star**2)/k + p*(i_star - delta*k)
assert sp.simplify(sp.diff(H, Y) - sp.diff(P(Y), Y)*q) == 0

# Gateaux derivative under mixture: Y_eps = (1-ε)Y + ε Y_nu
eps, Y_nu = sp.symbols('eps Y_nu', real=True)
Gateaux = sp.diff((P((1-eps)*Y + eps*Y_nu)*q), eps).subs(eps, 0)
expected = sp.diff(P(Y), Y) * q * (Y_nu - Y)
assert sp.simplify(Gateaux - expected) == 0
\end{sympycheck}
...
% Close the roles cheat-sheet list and box
  \item \emph{Direct coupling (Flat derivative):} $\displaystyle \int \delta\_m \mathcal H\,\mathrm dm$; here by envelope $\int \delta\_m \pi\,\mathrm dm = e^{x+z}k^\alpha\,Y\,P'(Y)$, which for isoelastic $P$ reduces to $-\eta P(Y)e^{x+z}k^\alpha$. This is distinct from transport.
\end{itemize}
\end{tcolorbox}
...
\node[box, below=of hjb] (direct) {Direct coupling\\$\int \delta\_m \mathcal H\,\mathrm dm$};
\node[box, below=of pop] (me) {ME residual};
\draw[->, thick] (hjb) -- (me);
\draw[->, thick] (pop) -- (me);
\draw[->, thick] (direct) -- (me);
...
\begin{tcolorbox}[didacticstyle]
  	extbf{Non-uniform grids and boundary diagnostics.}
On non-uniform $k$-grids, use cell widths $\Delta k_j$ in the discrete divergence $\big(F_{j+1/2}-F_{j-1/2}\big)/\Delta k_j$ and average velocities to interfaces (harmonic/biased averages improve robustness when $\Delta k$ varies sharply). Always check:
\begin{itemize}[leftmargin=1.15em,itemsep=0.2em]
  \item Mass: $\sum_j m_j$ is conserved up to the right-boundary outflow (reflecting at $k=0$ enforces $F_{1/2}=0$).
  \item Positivity: $m_j\ge 0$ (monotone upwind flux + small floor safeguard if needed).
  \item CFL: $\max_j |i^*_j-\delta k_j|\, \Delta t / \min_j \Delta k_j \le 1$.
\end{itemize}
...
\begin{tcolorbox}[mathstyle]
  	extbf{On identifiability.} Because $\dmU$ appears only through $\partial_\kappa\dmU, \partial_\zeta\dmU, \partial^2_{\zeta\zeta}\dmU$, adding constants leave the ME invariant. An anchoring penalty $\mathcal{P}_{\mathrm{anchor}}=\big(\int \dmU\,\diff m\big)^2$ fixes the gauge.
\end{tcolorbox}
...
% Schematic log-linear decay for HJB and ME residuals
\addplot[color=wongBlue, thick] coordinates {(0,0) (5,-0.9) (10,-1.8) (15,-2.7) (20,-3.6) (25,-4.4) (30,-5.1) (35,-5.7) (40,-6.3) (45,-6.8) (50,-7.2)};
\addlegendentry{$\|\mathcal{R}_{\mathrm{ME}}\|$}
\addplot[color=wongVerm, dashed, thick] coordinates {(0,-0.2) (5,-1.1) (10,-2.0) (15,-2.9) (20,-3.8) (25,-4.6) (30,-5.3) (35,-5.9) (40,-6.4) (45,-6.9) (50,-7.3)};
\addlegendentry{$\|\mathcal{R}_{\mathrm{HJB}}\|$}
\end{axis}
\end{tikzpicture}
...
Under isoelastic demand $P(Y)=Y^{-\eta}$, the marginal revenue wedge is $W(k,z,x,m) = -\eta\, P(Y)\, e^{x+z}k^\alpha$. The magnitude of this negative externality
is strictly increasing in $\eta$ (holding $Y$ fixed) if and only if $\ln Y < 1/\eta$.
\end{lemma}
\begin{proof}
Consider the magnitude $|W| = \eta\, Y^{-\eta}\, e^{x+z}k^\alpha$. We analyze the direct effect by taking the partial derivative with respect to $\eta$:
\[
...
\]
Since $|W|>0$ and $\eta>0$, the sign is positive if and only if $1/\eta > \ln Y$ (or $Y < e^{1/\eta}$). This condition holds if $Y=1$ (normalization of units) or generally when $Y$ is not excessively large relative to $1/\eta$.
Since $|W|>0$ and $\eta>0$, the sign is positive if and only if $1/\eta > \ln Y$ (or $Y < e^{1/\eta}$).
\end{proof}
...
theorem deriv_wedge_wrt_eta (Y eta : R) (hY : 0 < Y) :
...
\begin{tcolorbox}[didacticstyle]
  	extbf{Interpretation.} A higher $\eta$ (steeper demand curve) amplifies the negative feedback from aggregate output onto individual firm revenues, provided the exact condition $\ln Y < 1/\eta$ holds. This is the \emph{direct effect} (holding $Y$ fixed). Note that the full general equilibrium effect involves $\mathrm dY/\mathrm d\eta$, which introduces feedback; however, the direct effect analyzed here captures the primary incentive shift leading to more cautious investment.
\end{tcolorbox}
...
theorem deriv_inv_mul_const (k phi : R) (hphi : phi ≠ 0) :
...
%========================
% Appendix: Derivations and Technical Lemmas
%========================
\section*{Appendix Preliminaries: Derivations and Technical Lemmas}

\subsection*{A.6 Functional It\^o link: measure flow to transport operator $\mathcal T$}\label{app:functional-ito-transport}

\begin{tcolorbox}[mathstyle]
  	extbf{Outline (conditional on $x$).}
Let $(\kappa_t,\zeta_t)$ follow the controlled drift-diffusion with drift $b(\xi_t)=(i^*-\delta\kappa_t,\,\mu_z(\zeta_t))$ and diffusion only in $z$ with variance $\sigma_z^2$. Consider the master value $U(k,z,x,m)$ with Lions derivative $\Dm U(\xi;k,z,x,m)$. By Functional It\^o applied to the lift $\tilde U(X,m)$ and the measure flow $m_t=\mathrm{Law}(\xi_t)$,
\begin{itemize}[leftmargin=1.15em,itemsep=0.25em]
  \item the own-state part yields $U_k(i^*-\delta k)+\Lz U+\Lx U$;
  \item the measure-flow part contributes the average of the generator applied componentwise to the vector field $\Dm U$ evaluated at $\xi$.
\end{itemize}
Thus, conditional on $x$, the population-transport term equals
\[
\int \mathcal T[\Dm U](\xi)\, m(\mathrm d\xi)
\equiv \int \Big[(i^*-\delta\kappa)\, \partial\_\kappa(\Dm U)\!\_\kappa
 + \mu\_z(\zeta)\, \partial\_\zeta(\Dm U)\!\_\zeta
 + \tfrac12\sigma\_z^2\, \partial^2\_{\zeta\zeta}(\Dm U)\!\_\zeta\Big] m(\mathrm d\xi),
\]
matching Lemma~\Cref{lem:transport-bookkeeping}. Reflecting at $k=0$ ensures boundary fluxes vanish in the adjoint pairing.
\end{tcolorbox}

\begin{tcolorbox}[didacticstyle]
  	extbf{Economic reading.} The transport term aggregates marginal impacts from the \emph{movement of mass} across states under optimal behavior. It is the general-equilibrium drift: when many firms invest (or disinvest), the cross-sectional motion feeds back into each firm’s value through $\Dm U$.
\end{tcolorbox}
...
theorem deriv_lognormal_var_factor (s : R) :
  ...
  have h1 : deriv (fun x => exp (2*x)) s = 2 * exp (2*s) := by
    simpa using (hasDerivAt_exp (2*s)).comp s (hasDerivAt_id' s).const_mul 2
...
\medskip\noindent\textbf{Hardening notes.} The SymPy snippets explicitly use the assumptions/refine/ask stack to guard simplifications on the intended domain (e.g., $Y>0$ for real powers and $\eta>0$ for isoelastic demand). When simplifying identities, we first declare assumptions (\texttt{assuming(...)} or \texttt{Q.pos(...)}) and then query with \texttt{ask} before applying \texttt{refine}, so equalities are asserted only under those hypotheses. Differentiation and algebraic reductions rely on standard \texttt{diff}/\texttt{simplify}/\texttt{expand} routines; series or limit checks (when present) use \texttt{series}/\texttt{limit} under the same assumptions.
...
\begin{tcolorbox}[didacticstyle]
	extbf{Notes.} The lemmas use $\mathrm{rpow}$ and standard calculus from \texttt{mathlib4}. They require $Y>0$ for real-exponent laws. The SymPy checks in \Cref{app:verification} independently validate the same identities numerically/symbolically. In Lean, the corresponding facts are provided by canonical calculus and order-theory lemmas (e.g., \texttt{HasDerivAt} rules for products and real powers under positivity hypotheses, and monotonicity classes \texttt{Monotone}/\texttt{Antitone}). Our sketches state the necessary assumptions so these lemmas apply without additional glue.
\end{tcolorbox}
...
\noindent\emph{Special cases.} When $\psi=1$ (unit EIS) or $\gamma=1$ (log utility), the risk-adjustment coefficient $(1-\gamma)(1-1/\psi)$ in \Cref{eq:ez-agg} vanishes and the utility-channel exposure collapses accordingly, consistent with the CRRA/log special cases derived in Section~\ref{sec:crra}.
...
\addcontentsline{toc}{section}{Appendix H: Context-Engineered Prompt and Minimal Deep-FBSDE Follow-through}
