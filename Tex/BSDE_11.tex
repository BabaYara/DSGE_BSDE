...
theorem chain_rule_composition (Φ : E → F) (H : F → G) (X : E)
  ...
  fderiv ℝ (H ∘ Φ) X = (fderiv ℝ H (Φ X)).comp (fderiv ℝ Φ X) := by
...
We apply the Flat chain rule (\Cref{lem:chain_flat}). Let $G(Y)=P(Y)$ and $\Phi(m)=Y(m,x)=\int \varphi(\xi)\,m(\diff\xi)$, where $\varphi(\xi)=e^{x+\zeta}\kappa^\alpha$ is the output of the marginal firm $\xi$.
The Flat derivative of the linear functional $\Phi(m)$ is the integrand:
\[
\tfrac{\delta\Phi}{\delta m}(m)(\xi) = \varphi(\xi) = e^{x+\zeta}\kappa^\alpha.
\]
By the chain rule for $G\circ\Phi$ (Flat derivative):
\[
\frac{\delta}{\delta m}[G\!\circ\!\Phi](m)(\xi)=G'(Y)\,\varphi(\xi)=P'(Y)\,e^{x+\zeta}\kappa^\alpha.
\]
The revenue $R$ is $e^{x+z}k^\alpha \cdot (G\circ\Phi)(m)$. Thus, its Flat derivative is:
\[
\delta\_m R(\xi;\,k,z,x,m) = e^{x+z}k^\alpha \cdot \tfrac{\delta}{\delta m}[G\!\circ\!\Phi](m)(\xi) = e^{x+z}k^\alpha\,P'(Y)\,e^{x+\zeta}\kappa^\alpha.
\]
...
\end{align*}
  where the last line uses the Flat chain rule in \Cref{lem:chain_flat}. Taking expectations under the pricing measure with short rate $r(x)$ and imposing stationarity produces the stationary master equation (ME).
