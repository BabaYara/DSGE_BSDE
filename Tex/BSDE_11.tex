...
%
% {
%   "target_subsection": "Multiple (Consolidated 5-Iteration Revision)",
%   "reasons": [
%     "Improve mathematical rigor by refining proof sketches (Hamiltonian convexity) and expanding derivations (Functional Itô).",
%     "Enhance verification coverage by adding specific Lean4 lemmas (quadratic derivatives, EZ limits) and ensuring SymPy checks cover edge cases (depreciation).",
%     "Strengthen computational completeness by refining pseudocode clarity (boundary conditions, gradient calls) and adding practical implementation details (loss tables, implementation hooks).",
%     "Improve didactic clarity by restructuring the flow (moving EZ limit discussions to appendices) and ensuring clear citations for specific formulations (EZ normalization).",
%     "Enhance global hygiene, formatting consistency, and build robustness (Preamble configuration, bibliography completeness)."
%   ],
%   "build_notes": {
...
Fix $(k,z,x,m)$ and write $J(i;p)\equiv \pi(k,i,z,x,m)+p\,(i-\delta k)$. On each branch $i\gtrless 0$ with quadratic $h(i,k)=\tfrac{\phi_\pm}{2}\,i^2/k$, the $i$-dependent part of $J$ equals $-(\phi_\pm/(2k))\,i^2+(p-1)\,i$, which is a strictly concave quadratic in $i$ with unique maximizer $i^*(p)=(k/\phi_\pm)\,(p-1)$ when it respects the branch. Evaluating at $i^*(p)$ yields a branch value of the form $a\_\pm(k)\,p^2+b\_\pm(k)\,p+c\_\pm(k)$ with $a\_\pm(k)=k/(2\phi_\pm)>0$, hence convex in $p$. The Hamiltonian is the pointwise maximum of these two convex branch values (and any linear boundary piece if complementarity binds), therefore convex in $p$. The envelope identity holds: $\partial\_p\mathcal H = i^*(p)-\delta k$, as verified symbolically above.
\end{proof}
