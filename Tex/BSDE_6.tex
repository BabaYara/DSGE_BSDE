...
\usepackage{newtxmath}         % High-quality math fonts
...
\usepackage{xcolor}
% (distill* colors already defined above)

\definecolor{codeBg}{RGB}{248, 250, 252}          % Code background
...
\usetikzlibrary{intersections, patterns, decorations.pathreplacing}
...
\captionof{figure}{\textbf{The Physics of Self-Consistency Bias.} (A) EM's asymmetric error correlates with the training gradient, inducing bias. (B) Symmetric schemes (Itô-Heun) center the error, canceling the leading bias term.}
\label{fig:Bias_Physics_R3}
\end{figure}
